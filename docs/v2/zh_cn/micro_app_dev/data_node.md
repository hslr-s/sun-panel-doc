...
数据权限可以分为三种（`读`、`写`、`(透传)使用`），所有级别的数据节点都支持在 API 的 [request](/v2/zh_cn/micro_app_dev/api.html#network-request) 透传请求中模板替换使用，这样既保证了敏感数据不外漏在前端，还能安全使用数据节点中的敏感数据，来保证透传接口的正常调用。读、写权限请参考：
