...
- **其他** 
    ...
    - [移除] 智能选择内外地址和默认地址打开，因测试期间反馈较少，暂时移除此功能准备发布正式版本。将会在下一个大版本(v1.8.0)的Beta版本再次开启测试
    - [移除] 页面版本缓存技术（Cache Storage），经测试期间出现致命问题（更新后会导致页面无法访问）普通用户不易解决，本着易用并减少用户使用难度的理念，已移除此功能（移除后不能正常使用请看[解决方案](../faq/faq#v1_7_0_beta_abnormal_faq)）
